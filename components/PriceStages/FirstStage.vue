        ...
        <div v-for="(stage, index) in stages" :key="index" class="stages-accordion-group border-b border-[#878787B2]" :class="{ 'expanded': stage.isExpanded }">
          <div class="stages-accordion-menu" @click="toggleMenu">
            <div class="stage-title grid grid-cols-6 sm:grid-cols-5 lg:grid-cols-4 items-center w-full px-4 py-5 2xl:py-7 text-left hover:bg-white/10 transition duration-700 ease-in-out">
              <h3 class="text-xs min-[350px]:text-sm min-[400px]:text-lg md:text-base lg:text-xl xl:text-2xl col-span-2">{{ stage.title }}</h3>
              <h4 class="text-xs min-[350px]:text-sm min-[400px]:text-lg md:text-base lg:text-lg xl:text-2xl col-span-3 sm:col-span-2 lg:col-span-1 justify-self-center">от {{ stage.price }} тг</h4>
              <img
...
const stages = ref([
  ...
  {
    title: 'Прототип',
    desc: 'Черно-белый функциональный прототип сайта, чтобы определить расположение контента и функционала на страницах без учета изображений. Это позволит четко представить, как будет выглядеть и функционировать сайт на ранней стадии проекта.',
    price: 30000,
    rotation: 0,
    isExpanded: false
  },
  {
    title: 'Дизайн',
    desc: 'После утверждения функционального прототипа, мы переходим к созданию визуальной части сайта. Представляем несколько дизайн-концепций для различных блоков, и после подтверждения разрабатываем дизайн для всего сайта.',
    price: 40000,
    rotation: 0,
    isExpanded: false
  },
  {
    title: 'Разработка',
    desc: 'Создание сайта на популярных конструкторах, альтернативно - прямая разработка с участием лучших IT-специалистов. В любом случае, мы гарантируем высокое качество и соответствие вашим требованиям.',
    price: 55000,
    rotation: 0,
    isExpanded: false
  },
